...
tree2 <- read.beast.newick("output/pufferfish_FcBhDc.tre")
tree3 <- read.beast.newick("output/pufferfish_FhBcDc.tre")
tree4 <- read.beast.newick("output/pufferfish_FhBhDc.tre")

tree1 <- tree2
tree1@data$mean_netdiv <- rep(0.048266973607553554, length(tree1@data$mean_netdiv))
tree1@data$mean_psi <- rep(0.005204945359362939, length(tree1@data$mean_psi))
...
ntips <- length(tree2@phylo$tip.label)
...
p1b <- revts(ggtree(tree1, aes(color = mean_psi)) +
  ...
  coord_geo(xlim = c(-65, 0), size = 3, ylim = c(-2, ntips+2),
...
ggsave("figures/pufferfish_4models.pdf", width = 160, height = 140, units = "mm")
